...
root.title("Myopia Detection System")
...
#
lbl = tk.Label(root, text="Welcome to Detection of Myopia Disease System", font=('times', 25,' bold '), height=1, width=75,bg="skyblue",fg="black")
lbl.place(x=0, y=3)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="black",fg="white")
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=0, y=120)
...
#    global fn
    ...
    if fn!="":
        ...
        if plant == 0:
            Cd="Normal Image Detected \n Symptoms are : The given image is a normal eye image."
        elif plant == 1:
            Cd="High Myopia Detected\n Symptoms are : An increased risk of developing other eye problems, \n such as glaucoma, cataracts, and retinal detachment"
            #print('Retinal Disease Detected')
            
            # sms_send()
        elif plant == 2:
            Cd="Pathological Myopia Detected\n Symptoms are :Flashes of light - Floaters - Loss of peripheral vision -\n Nearsightedness that worsens rapidly "
...
def update_label(str_T):
    #clear_img()
    result_label = tk.Label(root, text=str_T, width=50, font=("bold", 25), bg='#F5F5F5', fg='black')
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='C:/Users/mayur/Desktop/Pathological Myopia Detection 50% code/Pathological Myopia Detection 100% code/test_set', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
